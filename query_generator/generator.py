...
from operator_generator_strategies.distinct_operator_strategies.distinct_sink_strategy import DistinctSinkStrategy
from operator_generator_strategies.distinct_operator_strategies.distinct_source_strategy import SourceOperator, DistinctSourceStrategy
...
class QueryGenerator:
    def __init__(self, config: GeneratorConfig):
        self._config = config
        self._equivalentOperatorGenerators: List[BaseStrategy] = config.equivalentOperatorGenerators
        self._distinctOperatorGenerators: List[BaseStrategy] = config.distinctOperatorGenerators
        self._queries: List[Query] = []

    def generate(self) -> List[Query]:
        # self.__inject_source_operators()
        sourceGenerator = DistinctSourceStrategy()
        ...
        while len(self._queries) < self._config.numberOfQueries:
            ...
            sinkOperator = DistinctSinkStrategy().generate(outputSchema)
